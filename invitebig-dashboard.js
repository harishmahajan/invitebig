...
function ReBindBookingsTable(e) {
    e.find("tr td").off("click").click(function(e) {
        ...
        $("#mainModalHeader").empty().append("Booking Details<div class='pull-right' style='font-size:12px'><a href='/booking/" + t + "'>Full Screen & Print</a></div>"), $("#mainModalAcceptBtn").empty().append("OK").css({
            ...
            }), $(".close").off("click").click(function(e) {
                $("#mainModalBody").empty(), $("#mainModal").modal("hide")
            })
        })
        ...
        $("#mainModalHeader").empty().append("Are you sure?"), $("#mainModalAcceptBtn").empty().append("Approve").css({
            display: "inline"
            ...
            Post(r).then(function(e) {
                if (e["result"] == "success") {
                    ...
                    r.find("td:nth-last-child(2)").empty().append("Pending Payment"), $("tr[data-booking=" + n + "]").first().remove(), r.find("td").last().empty().prepend("<small><small><em>Refresh page to change this</em></small></small>"), $("#venueBookingsConfirmedTable").find("tbody").append(r), $("#mainModalBody").empty(), $("#mainModal").modal("hide"), $("#mainModal").empty()
                    location.reload();
        ...
        $("#mainModalHeader").empty().append("Are you sure?"), $("#mainModalAcceptBtn").empty().append("Deny").css({
            display: "inline"
        }), $("#mainModalCloseBtn").empty().append("Not Yet").css({
            display: "inline"
    ...
    Post(n).then(function(e) {
        if (e["result"] == "success") {
            ...
            for (var o = 0; o < results.length; o++) {
                var u = "<tr data-booking='" + SanitizeAttr(results[o].bookingID) + "'><td>" + "->"+results[o].timezone+ "</td>" + "<td>" + results[o].customer + "</td><td>" + "" + Math.ceil(results[o].duration / 3600 * 100) / 100 + " hrs</td>" + "<td>" + FormatDollars(results[o].total, results[o].currency) + "</td><td>" + FormatDollars(results[o].income, results[o].currency) + "</td>" + "<td>" + results[o].status + "</td></tr>";
