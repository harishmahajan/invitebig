					...
					if ($stmt->prepare("SELECT resourceid, startminute, stopminute FROM resources_hours WHERE resourceid = ?"))
					{
						...
						//		print_r($stoparray);
							...
							// 	echo "\r\n";
								...
								// if ($stmt2->prepare("SELECT resourceid FROM booking_resources WHERE resourceid = ".$resourceid." and venueid = ".$arr['venueid'] . " and starttime IN ( ". $startcheck1 . "," . $startcheck2 . "," . $startcheck3 ." and stoptime IN ( ". $stopcheck1 . "," . $stopcheck2 . "," . $stopcheck3 ))
								// {
								// 	$stmt2->bind_param("i",$arr['books'][$i]['id']);
								// 	$stmt2->execute();
								// 	$stmt2->bind_result($resourceid);
