...
{	
	...
	if (isset($arr['venueid']))
	{
		...
		{		
			...
			//print_r($resourceid_ary);
			//print_r($resourceid_ary1);
			//exit();
			//$arr['books'];
			//exit();
			...
			foreach ($arr['books'] as &$resource)
			{
				...
				if ($stmt->prepare("SELECT br.starttime, br.stoptime, s.status, r.cleanuptime, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid LEFT JOIN resources AS r ON r.resourceid = br.resourceid WHERE br.resourceid = ? AND br.starttime < ? AND br.stoptime >= ? AND (s.status = 'Paid' OR s.status = 'Pending Payment' OR s.status = 'Imported' OR b.status = 'Past Due')"))
				{
					...
					while($stmt->fetch())
					{
						...
						$resource['schedule'][] = array("status"=>"busy","start"=>$adjusted_start,"stop"=>$adjusted_stop);
						
						// block parent if this child is booked
						foreach ($arr['books'] as &$r)
						{
							if (in_array($r['id'],$resource['parents']))
			...
			//print_r($arr['books']);
			//exit();
			$stmt1 = $GLOBALS['db']->stmt_init();
			$timestamp1 = strtotime(date("d/m/Y"));
			//if ($stmt1->prepare("SELECT bookingid, venueid, eventid, userid, timestamp, start, stop FROM booking"))
			if ($stmt1->prepare("SELECT bookingid, venueid, eventid, resourceid, starttime, stoptime FROM booking_resources where venueid = " . $arr['venueid']))
			{
				//$stmt->bind_param("i",$arr['books'][$i]['id']);
				$ts="";
				$stmt1->execute();
				$stmt1->bind_result($bookingid, $venueid, $eventid, $resourceid, $starttime, $stoptime);
				$startarray=array();
				$stoparray=array();
				$bookingid_ary=array();
				$resourceid_ary1=array();

				$cnt=0;

				while($stmt1->fetch())
				{
					//$tstamp = $timestamp;date("Y-m-d H:i:s", $unixTimestamp);
					//$startarray[$cnt] = $start;
					//$stoparray[$cnt] = $stop;
					$bookingid_ary[$cnt]=$bookingid;
					//$resourceid_ary1[$cnt]=$resourceid;
					// echo $tstamp;
					//echo "\n";
					// echo "start:- ".$strt;
					// echo "\n stop:- ";
			...
			//echo "ARRRRRY\r\n";
			//print_r($bookingid_ary);
			//exit();
			// Added By Harish 28/Apr/2016
			// To get the starttime & stoptime of 
			$stmt1 = $GLOBALS['db']->stmt_init();
			//if ($stmt1->prepare("SELECT timestamp,start,stop FROM booking where bookingid=".$resource['id']))

			//echo  "resourceid".$resource['id']; //125
			//echo "venueid".$arr['venueid']; //2
			//echo "bookingid".
			//exit();
			$timestamp = strtotime(date("Y/m/d"));
			//echo $timestamp;
			if ($stmt1->prepare("SELECT bookingid, starttime, stoptime, resourceid FROM booking_resources where venueid = ".$arr['venueid']. " and starttime >= ".$timestamp))
			{
				//$stmt->bind_param("i",$arr['books'][$i]['id']);
				$ts="";
				$stmt1->execute();
				$stmt1->bind_result($bookingid,$start,$stop,$resourceid);
				$startarray=array();
				$stoparray=array();
				// $bookingid_ary=array();
				// $resourceid_ary1=array();
			...
			//print_r($bookingid_ary);
			 //print_r($resourceid_ary);
			 //print_r($resourceid_ary1);
			 //echo "SELECT bookingid, starttime, stoptime, resourceid FROM booking_resources where resourceid = ".$resource['id']." and venueid = ".$arr['venueid'];
			 //exit();
			// print_r($arr['books']);
			// exit();
			...
			{	
				if ($arr['books'][$i]['timeslots'] == 0)
				{
					...
					if ($stmt->prepare("SELECT resourceid, startminute, stopminute FROM resources_hours WHERE resourceid = ?"))
					{
						...
						while($stmt->fetch())
						{
							...
							$stopcheck1=$firstdatetime->getTimestamp() + ($stopminute*60);
							$stopcheck2=$firstdatetime->getTimestamp() + ($stopminute*60) - 604800;
							...
							// echo $firstdatetime->getTimestamp() + ($startminute*60) - 604800;
							// echo "=>";
							// echo $firstdatetime->getTimestamp() + ($startminute*60);
							// echo "\r\n";

							// if($ts > $firstdatetime->getTimestamp() + ($startminute*60) - 604800 && $ts < $firstdatetime->getTimestamp() + ($stopminute*60) - 604800)
							// {

							// }


							//echo $startcheck1."=".$startcheck2."=".$startcheck3."=>"."1462316400 = R_Id".$resourceid;
							//	echo "\r\n";
							//echo $stopcheck1."=".$stopcheck2."=".$stopcheck3."=>"."1462359600 = R_Id".$resourceid;
							//	echo "\r\n";

							//	print_r($startarray);
						//		print_r($stoparray);
								//print_r($resourceid_ary); 1462161220 | 1462316400 | 1462359600 
							// else{
							 // echo $startcheck;
							 // echo "=>Start:";
							 // echo "1461884400";  1462185574 | 1462230000 | 1462267800 
 							
							 // echo $stopcheck;
							 // echo "=>stop:";
							 // echo "1461927600"; 1462316400 | 1462359600 
							 // echo "\r\n";

							//if (( in_array($resourceid, $resourceid_ary) && in_array($startcheck1, $startarray) || in_array($startcheck2, $startarray) || in_array($startcheck3, $startarray)) && (in_array($stopcheck1, $stoparray) || in_array($stopcheck2, $stoparray) || in_array($stopcheck3, $stoparray) ))
							//if (( in_array($startcheck1, $startarray) || in_array($startcheck2, $startarray) || in_array($startcheck3, $startarray)) && (in_array($stopcheck1, $stoparray) || in_array($stopcheck2, $stoparray) || in_array($stopcheck3, $stoparray) ))
							//{

							// echo "Colse=".$startcheck1."=".$startcheck2."=".$startcheck3."=>"."1462230000 = R_Id".$resourceid;
							// 	echo "\r\n";
							// echo "Colse=".$stopcheck1."=".$stopcheck2."=".$stopcheck3."=>"."1462267800 = R_Id".$resourceid;
							// 	echo "\r\n";


							// *******************************************************************************

							//$sql = "SELECT br.starttime, br.stoptime, s.status, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = ".$resourceid." AND br.starttime IN (". $startcheck1 . "," . $startcheck2 . "," . $startcheck3 .") AND br.stoptime IN (". $stopcheck1 .",". $stopcheck2 .",". $stopcheck3 . ") AND s.status = 'Paid'";
							...
							//while($row = $result->fetch_assoc()) {
								//echo "If Check";			
							//			$arr['books'][$i]['schedule'][] = array("status"=>"close","start"=>$firstdatetime->getTimestamp() + ($startminute*60) - 604800,"stop"=>$firstdatetime->getTimestamp() + ($stopminute*60) - 604800);
							//			$arr['books'][$i]['schedule'][] = array("status"=>"close","start"=>$firstdatetime->getTimestamp() + ($startminute*60),"stop"=>$firstdatetime->getTimestamp() + ($stopminute*60));
							//			$arr['books'][$i]['schedule'][] = array("status"=>"close","start"=>$firstdatetime->getTimestamp() + ($startminute*60) + 604800,"stop"=>$firstdatetime->getTimestamp() + ($stopminute*60) + 604800);									
										//$flag = 0;
							//}
							//else{
							//if($flag == 1){	
									...
									//}
							// ******************************************************************************* 
							$stmt4 = $GLOBALS['db']->stmt_init();
							if ($stmt4->prepare("SELECT br.starttime, br.stoptime, s.status, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = ".$resourceid." AND br.starttime IN (". $startcheck1 . "," . $startcheck2 . "," . $startcheck3 .") AND br.stoptime IN (". $stopcheck1 .",". $stopcheck2 .",". $stopcheck3 . ") AND s.status = 'Paid'"))
							{
								//$stmt->bind_param("i",$arr['books'][$i]['id']);
								$ts="";
								$stmt4->execute();
								$stmt4->bind_result($starttime,$stoptime,$status,$cleanuptime);
								...
								//$GLOBALS['db']->CloseConn();
								 ...
								 //$stmt2 = $GLOBALS['db']->stmt_init();
								  //echo "SELECT br.starttime, br.stoptime, s.status, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = ".$resourceid." AND br.starttime IN (". $startcheck1 . "," . $startcheck2 . "," . $startcheck3 .") AND br.stoptime IN (". $stopcheck1 .",". $stopcheck2 .",". $stopcheck3 . ") AND s.status = 'Paid'".";";
								  //echo "\r\n";
								 //$stmt->prepare("SELECT br.starttime, br.stoptime, s.status, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = ".$resourceid." AND br.starttime IN (". $startcheck1 . "," . $startcheck2 . "," . $startcheck3 .") AND br.stoptime IN (". $stopcheck1 .",". $stopcheck2 .",". $stopcheck3 . ") AND s.status = 'Paid'");
								 //$stmt2->prepare("SELECT br.starttime, br.stoptime, s.status, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = ".$resourceid." AND br.starttime IN (". $startcheck1 . "," . $startcheck2 . "," . $startcheck3 .") AND br.stoptime IN (". $stopcheck1 .",". $stopcheck2 .",". $stopcheck3 . ") AND s.status = 'Paid'");

								 //$stmt2->prepare("SELECT br.starttime, br.stoptime, s.status, br.cleanuptime, br.resourceid FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = 80 AND (br.starttime = '1461711600' OR br.starttime = '1462316400' OR br.starttime = '1462921200') AND ( br.stoptime = '1462359600' OR  br.stoptime = '1461754800' OR  br.stoptime = '1462964400') AND s.status = 'Paid'");
								 //SELECT br.starttime, br.stoptime, s.status, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid WHERE br.resourceid = 80 AND br.starttime IN (1461711600,1462316400 ,1462921200) AND br.stoptime IN (1462359600,1461754800,1462964400) AND s.status = 'Paid'
									//$stmt2->prepare("SELECT bookingid from booking");								 					 
								 	//$stmt2->bind_param("i",$starttime);
								 	//echo "If Check \r\n";
								 	//$stmt2->execute();
								 	//$stmt2->bind_result($bookingid);
				...
				{
					$stmt = $GLOBALS['db']->stmt_init();
					if ($stmt->prepare("SELECT startminute, stopminute, rate, combinable FROM resources_slots WHERE resourceid = ?"))
					{
						$stmt->bind_param("i",$arr['books'][$i]['id']);
						$stmt->execute();
						$stmt->bind_result($startminute, $stopminute, $rate, $combinable);
						while($stmt->fetch())
						{
							$arr['books'][$i]['schedule'][] = array("status"=>"open","start"=>$firstdatetime->getTimestamp() + ($startminute*60) - 604800,"stop"=>$firstdatetime->getTimestamp() + ($stopminute*60) - 604800);
							$arr['books'][$i]['schedule'][] = array("status"=>"open","start"=>$firstdatetime->getTimestamp() + ($startminute*60),"stop"=>$firstdatetime->getTimestamp() + ($stopminute*60));
...
function SnapToTimeslot(&$startmin, &$stopmin, &$resrates)
{
	...
	while ($modified)
	{
		...
		if (!$goodstart)
		{
			//echo "start: ".$startmin." stop: ".$stopmin." cnt: ".count($resrates)."\n";
			$best = 9999999;
			for ($i=0; $i<count($resrates); $i++)
			{
...
function FindParents($id)
{
	...
	$stmt = $GLOBALS['db']->stmt_init();
	if ($stmt->prepare("SELECT parentid FROM resources_relationships WHERE childid = ? AND (relation = 'inside_linked' OR relation = 'inside_unlinked') ORDER BY childid"))
	{		
		$stmt->bind_param("i",$id);
		$stmt->execute();
