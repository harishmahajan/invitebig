	...
	if (isset($arr['venueid']))
	{
		...
		{		
			...
			foreach ($arr['books'] as &$resource)
			{
				...
				// Harish:-
				// Below array is used to check liked parents 
				$resource['checkparents'] = checkparents($resource['id']);

				//$resource['children'] = FindChildren($resource['id'],true); Commented By Harish 
				
				// Harish:-
				// Bellow array is used to check linked parents
				$resource['children'] = checkChildren($resource['id'],true);
				$resource['checkchildren'] = checkChildren($resource['id'],true);
				...
				if ($stmt->prepare("SELECT br.starttime, br.stoptime, s.status, r.cleanuptime, br.cleanuptime FROM booking_resources AS br LEFT JOIN booking AS s ON s.bookingid = br.bookingid LEFT JOIN resources AS r ON r.resourceid = br.resourceid WHERE br.resourceid = ? AND br.starttime < ? AND br.stoptime >= ? AND (s.status = 'Paid' OR s.status = 'Pending Payment' OR s.status = 'Imported' OR s.status = 'Past Due')"))
				{
					...
					while($stmt->fetch())
					{
						...
						foreach ($arr['books'] as &$r)
						{
							if (in_array($r['id'],$resource['checkchildren']))
								{
									$r['schedule'][] = array("status"=>"busy","start"=>$adjusted_start,"stop"=>$adjusted_stop);												
								}
						}
		...
		for ($i=0; $i<count($arr['books']); $i++)
		{
			...
			$arr['books'][$i]['children'] = checkChildren($arr['books'][$i]['id'],false);
		}
		//echo $arr['venueid'];
		return $arr;
	}
...
function InsertBooking()
{
	...
	if (isset($_SESSION['booking']['id']))
	{
		foreach ($_SESSION['booking']['resources'] as $resource)
		{
			...
			$children = checkChildren($resource['id'],false);
			foreach ($children as $child)
			{
				$stmt = $GLOBALS['db']->stmt_init();
...
// Harish:-
// This function is used to get all the liked parent slots
...
// Harish:-
// This function is used to get all the liked and unliked parent slots
...
// Harish:-
// This function is used to get all the liked & unliked children slots
...
// Harish:-
// This function is used to get all the liked children slots only
function checkChildren($id,$unlinked = false)
{
	$arr = array();
	$arr2 = array();
	
	$stmt = $GLOBALS['db']->stmt_init();
	if ($stmt->prepare("SELECT r.childid FROM resources_relationships AS r LEFT JOIN resources AS r2 ON r2.resourceid = r.childid WHERE r2.status = 'new' AND r.parentid = ? AND (relation = 'inside_linked') ORDER BY parentid"))
	{		
		$stmt->bind_param("i",$id);
		$stmt->execute();
		$stmt->bind_result($cid);
	...
	foreach ($arr as $a)
	{
		$arr2 = array_merge($arr2,checkChildren($a,$unlinked));
	}
	$arr = array_merge($arr,$arr2);
